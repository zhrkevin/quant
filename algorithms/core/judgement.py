...
class ValuationSignal:
    ...
    def condition(self):
        selected_date = date(2026, 1, 1) if self.selected_date is None else deepcopy(self.selected_date)
        
        selected_stock_dividend = pl.read_excel(Config.Paths.DataPath / 'input' / 'dividends.xlsx', sheet_name='Sheet1')
        selected_stock_dividend = selected_stock_dividend.filter(pl.col('股票编号') == self.symbol).filter(pl.col('年份') <= selected_date.year)
        ...
        latest_stock_close = selected_stock_data['qfq_close'][-1]

        latest_dividend_ratio = selected_stock_dividend['分红'][-1] / latest_stock_close
        latest_shiye = selected_stock_dividend['公用事业'][-1]

        if latest_shiye:
            if 0.036 < latest_dividend_ratio <= 0.045:
                print(f'股票 {self.symbol} 日期 ({selected_date}) 低估')
            elif latest_dividend_ratio <= 0.036:
                print(f'股票 {self.symbol} 日期 ({selected_date}) 正常')
            else:
                print(f'股票 {self.symbol} 日期 ({selected_date}) 异常')
        else:
            if 0.04 < latest_dividend_ratio <= 0.06:
                print(f'股票 {self.symbol} 日期 ({selected_date}) 低估')
            elif 0.035 < latest_dividend_ratio <= 0.04:
                print(f'股票 {self.symbol} 日期 ({selected_date}) 正常')
            elif 0.03 < latest_dividend_ratio <= 0.035:
                print(f'股票 {self.symbol} 日期 ({selected_date}) 高估') 
            else:
                print(f'股票 {self.symbol} 日期 ({selected_date}) 异常')
...
class BottomSignal:
    ...
    def condition1(self):
        ...
        end_date = selected_date.replace(year=selected_date.year)

        stock_data = pl.read_parquet(Config.Paths.DataPath / 'input' / self.symbol / 'stock_day.parquet')
        selected_stock_data = stock_data.filter(pl.col('date') >= begin_date).filter(pl.col('date') <= end_date)
    ...
    def condition2(self):
        selected_date = date(2022, 11, 2) if self.selected_date is None else deepcopy(self.selected_date)
        ...
        selected_boll_month = boll_month.filter(pl.col('date') <= selected_date)
        latest_boll_month_lower = selected_boll_month['boll_lower'][-1]
        ...
        if latest_stock_low <= latest_boll_month_lower and latest_stock_high <= latest_boll_quarter_mid and latest_stock_low >= latest_boll_quarter_lower:
