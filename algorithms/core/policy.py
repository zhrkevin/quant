...
class AscendSignalWeekRotation:
    ...
    def __init__(self, symbol: str, selected_date: date = None, period: int = 2, adjust: str = 'raw'):
        self.symbol = symbol
    ...
    def condition(self):
        ...
        selected_date = date(2026, 1, 2) if self.selected_date is None else deepcopy(self.selected_date)
        limited_date = selected_date - timedelta(days=365*self.period)
...
class FirstBreakthroughSignal:
    ...
    def condition(self):
        ...
        selected_date = date(2026, 1, 2) if self.selected_date is None else deepcopy(self.selected_date)
        limited_date = selected_date - timedelta(days=365*self.period + 7)
        ...
        for i_date in selected_ma_week['date'][::-1]:
            ...
            if len(ma_week_20) > 1 and len(ma_week_60) > 1 and ma_week_20[-1] > ma_week_60[-1] and ma_week_20[-2] < ma_week_60[-2]:
                breakthrough_date = deepcopy(i_date)
