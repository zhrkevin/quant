...
class Plotting:
    ...
    def __init__(self, symbol: str, period: str = 'day', window: int = 300):
        self.period = period
    ...
    def plot(self):
        ...
        fig.add_traces(
            [
                go.Scatter(
                    x=self.stock_data['date'], y=self.macd_data['dif'], name='DIF', line={'color': 'black', 'width': 1}, showlegend=False,
                    hovertemplate=f'日期: %{{x|%Y-%m-%d}}<br>DIF: %{{y}}<extra></extra>'
                ),
                go.Scatter(
                    x=self.stock_data['date'], y=self.macd_data['dea'], name='DEA', line={'color': 'peru', 'width': 1}, showlegend=False,
                    hovertemplate=f'日期: %{{x|%Y-%m-%d}}<br>DEA: %{{y}}<extra></extra>'
                ),
                go.Bar(
                    x=self.stock_data['date'], y=self.macd_data['macd'], name='MACD', marker_color=self.macd_data['color'], showlegend=False,
