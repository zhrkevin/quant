...
class Analysis:
    ...
    @classmethod
    def run(cls):
        cls.raw = pl.read_parquet(Config['Paths']['DataPath'] / 'limitup' / 'today.parquet')        
        cls.preprocess()
        cls.judgement()
        cls.limitup.write_parquet(Config['Paths']['DataPath'] / 'limitup' / 'today-new.parquet')

    @classmethod
    def preprocess(cls):
        # 只做拆分，不考虑列的位置
        cls.limitup = pl.DataFrame({
            '代码': cls.raw['代码'],
            '名称': cls.raw['名称'],
            '最新价': cls.raw['最新价'],
            '换手率(%)': cls.raw['换手率'].round(5),
            '封板资金(亿)': (cls.raw['封板资金'] / 1e8).round(5),
            '成交额(亿)': (cls.raw['成交额'] / 1e8).round(5),
            '流通市值(亿)': (cls.raw['流通市值'] / 1e8).round(5),
            '总市值(亿)': (cls.raw['总市值'] / 1e8).round(5),
            '涨停天数': cls.raw['涨停统计'].str.split('/').list[0].cast(pl.Int64),
            '涨停板数': cls.raw['涨停统计'].str.split('/').list[1].cast(pl.Int64),
            '炸板次数': cls.raw['炸板次数'],
            '连板数': cls.raw['连板数'],
            '首次封板': cls.raw['首次封板时间'].str.strptime(pl.Time, '%H%M%S'),
            '最后封板': cls.raw['最后封板时间'].str.strptime(pl.Time, '%H%M%S'),
            '所属行业': cls.raw['所属行业'],
        })
        Logger.info(f'\n今日 {date.today()} 涨停个股：\n{cls.limitup}')
    ...
    @classmethod
    def judgement(cls):
        cls.limitup = cls.limitup.filter(
            (pl.col('换手率(%)') >= 15) & (pl.col('换手率(%)') <= 31) &
            (pl.col('总市值(亿)') >= 30) & (pl.col('总市值(亿)') <= 200)
        )
        Logger.info(f'\n今日 {date.today()} 筛选涨停个股：\n{cls.limitup}')
