...
class ValuationSignal:
    ...
    @classmethod
    def condition(cls):
        stock_dividend = pl.read_excel(Config['Paths']['DataPath'] / cls.product / 'dividends.xlsx')    
        stock_dividend = stock_dividend.filter(
            (pl.col('编号') == cls.symbol) & 
            (pl.col('年份') <= cls.today.year)
        )
        
        stock_data = pl.read_parquet(Config['Paths']['DataPath'] / cls.product / cls.symbol / 'day.parquet')
...
class BottomSignal:
    ...
    def condition2(cls):
        ...
        boll_month = pl.read_parquet(Config['Paths']['DataPath'] / cls.product / cls.symbol / 'boll_month.parquet')
        latest_boll_month_lower = boll_month.filter(pl.col('日期') <= cls.today)['Boll下轨'][-1]

        boll_quarter = pl.read_parquet(Config['Paths']['DataPath'] / cls.product / cls.symbol / 'boll_quarter.parquet')
