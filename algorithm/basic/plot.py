...
class Plotting:
    ...
    def __init__(self, symbol: str, period: str = 'day', window: int = 10000):
        self.period = period
        self.stock_data = pl.read_parquet(Config.Paths.DataPath / 'stock' / symbol / f'stock_{self.period}.parquet')[-window:]
        self.ma_data = pl.read_parquet(Config.Paths.DataPath / 'stock' / symbol / f'ma_{self.period}.parquet')[-window:]
        self.boll_data = pl.read_parquet(Config.Paths.DataPath / 'stock' / symbol / f'boll_{self.period}.parquet')[-window:]
        self.macd_data = pl.read_parquet(Config.Paths.DataPath / 'stock' / symbol / f'macd_{self.period}.parquet')[-window:]
...
class PlottingSpecific:
    ...
    def __init__(self, symbol: str):
        self.stock_data = pl.read_parquet(Config.Paths.DataPath / 'stock' / symbol / f'stock_day.parquet')
        self.ma_data_month = pl.read_parquet(Config.Paths.DataPath / 'stock' / symbol / f'ma_month.parquet')
        self.plot()
